...
class BertData():
    def __init__(self, args):
        ...
        self.rhetorical_role_map = {'FAC': 1, 'RLC': 2, 'ARG_PETITIONER': 3, 'ARG_RESPONDENT': 4, 'ISSUE': 5,
                                    'PRE_RELIED': 6, 'PRE_NOT_RELIED': 7,
...
def _format_to_bert(params):
    ...
    if (os.path.exists(save_file)):
        # msg.info('Ignore %s' % save_file)
        return

    bert = BertData(args)

    # msg.info('Processing %s' % json_file)
    ...
    # msg.info('Processed instances %d' % len(datasets))
    # msg.info('Saving to %s' % save_file)
